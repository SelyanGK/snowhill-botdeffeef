...
module.exports = {
  ...
  async checkAntiPing(message, client) {
    try {
      ...
      // Check if the no-ping role and mute role are set
      if (!antipingConfig.noPingRoleId || !antipingConfig.muteRoleId) return;
      ...
      if (hasViolation) {
        ...
        try {
          ...
          // Also add the mute role if needed
          await message.member.roles.add(antipingConfig.muteRoleId, 'Anti-ping violation');
          
          // Delete the violating message
          await message.delete();
          
          // Send warning to the user
          const warnMessage = antipingConfig.warnMessage || 'You have been muted for pinging a user with the no-ping role.';
          ...
          // Log the violation if a log channel is set
            ...
            if (logChannel) {
              await logChannel.send({
                embeds: [{
                  ...
                  description: `${message.author} has been muted for ${muteDuration} minutes for pinging a protected user.`,
                  fields: [
          ...
          logger.info(`Anti-ping violation: ${message.author.tag} (${message.author.id}) muted for ${muteDuration} minutes`);
        } catch (error) {
          logger.error(`Failed to mute user for anti-ping violation: ${error.message}`);
        }
      }
    } catch (error) {

...
const fortuneMessages = [
  ...
  'You have the ability to overcome difficult situations.',
...
// List of lucky numbers combinations
const luckyNumbers = [
  [7, 13, 22, 36, 45, 49],
  [3, 12, 27, 38, 42, 50],
  [8, 17, 26, 33, 41, 47],
...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('fortune')
    .setDescription('Get your fortune cookie message')
    .addBooleanOption(option =>
      option.setName('lucky_numbers')
        .setDescription('Include lucky numbers with your fortune')
        .setRequired(false)),
  
  cooldown: 60, // Long cooldown to make it feel special
  ...
  async execute(interaction) {
    try {
      ...
      const fortune = fortuneMessages[randomIndex];
      
      // Check if user wants lucky numbers
      const includeLuckyNumbers = interaction.options.getBoolean('lucky_numbers') || false;
      
      // Get lucky numbers if requested
      let luckyNumbersText = '';
      if (includeLuckyNumbers) {
        const luckyIndex = Math.floor(Math.random() * luckyNumbers.length);
        const numbers = luckyNumbers[luckyIndex];
        luckyNumbersText = `**Lucky Numbers:** ${numbers.join(', ')}`;
      }
      
      // Create and send the fortune cookie message
      const embed = new EmbedBuilder()
        .setTitle('🥠 Your Fortune Cookie')
        .setDescription(`*${fortune}*`)
        .setColor('#E8B564') // Fortune cookie color
        .setFooter({ text: 'Fortunes change with time, but wisdom is timeless' })
        .setTimestamp();
      
      // Add lucky numbers if requested
      if (includeLuckyNumbers) {
        embed.addFields({ name: 'Lucky Numbers', value: luckyNumbersText });
      }
      
      // Add a random Chinese word of wisdom (just for fun)
      const chineseProverbs = [
        ...
        '学无止境 - Learning is a treasure that will follow its owner everywhere',
        '天道酬勤 - Heaven rewards the diligent'
      ];
      
      const randomProverb = chineseProverbs[Math.floor(Math.random() * chineseProverbs.length)];
      embed.addFields({ name: 'Chinese Proverb', value: randomProverb });
      
      await interaction.reply({ embeds: [embed] });
      
      logger.info(`User ${interaction.user.tag} received a fortune cookie`);
    } catch (error) {
      logger.error(`Error executing fortune command: ${error.message}`);
      await interaction.reply({ 
        content: 'The fortune teller is currently unavailable. Please try again later.',
        ephemeral: true
      });
    }
  }
